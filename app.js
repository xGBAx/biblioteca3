...
document.getElementById('livroCreateForm').addEventListener('submit', async (e) => {
    ...
    const body = {
        ...
        autorId:          formData.get('autorId'),          // <-- nome correto do schema
...
document.getElementById('livroUpdateForm').addEventListener('submit', async (e) => {
    ...
    if (formData.get('autorId'))         body.autorId       = formData.get('autorId');
    if (formData.get('anoPublicacao'))   body.anoPublicacao = parseInt(formData.get('anoPublicacao'));
    if (formData.get('genero'))          body.genero        = formData.get('genero');
    if (formData.get('isbn'))            body.isbn          = formData.get('isbn');
    if (formData.get('quantidade') !== null && formData.get('quantidade') !== '') {
        body.quantidade = parseInt(formData.get('quantidade'));
    }
